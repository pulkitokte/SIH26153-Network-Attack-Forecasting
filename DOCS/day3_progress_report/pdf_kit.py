"""Pista-green PDF kit for the SIH26153 Day 3 progress report."""
...
CALLOUT_THEME = {
    ...
    "day3": ("Day 3 complete", DAY3_BG, DAY3_EDGE, FOREST_DARK),
    "next": ("Next priority", HexColor("#E8EEF4"), HexColor("#4A6FA5"), HexColor("#1E3A5F")),
    "presentation": ("Presentation note", HexColor("#F4F1E4"), HexColor("#8A7A3A"), HexColor("#5A4A18")),
}
...
def member_banner(name: str, role: str, status: str) -> Table:
    inner = [
        ...
        [P(role, "member_role")],
        [P(f"<b>Status:</b> {status}", "body_left")],
    ]
...
def draw_header_footer(canvas, doc, is_cover: bool = False) -> None:
    ...
    canvas.setFillColor(FOREST)
    canvas.drawRightString(PAGE_W - RIGHT, PAGE_H - 7 * mm, "Day 3 Progress Report  ·  Internal team handoff")

    canvas.setFillColor(WHITE)
    canvas.setFont("Calibri", 8)
    canvas.drawString(LEFT, 4.2 * mm, "SIH Team knowledge document  ·  04 Sep 2026")
    canvas.setFont("Calibri-Bold", 8)
...
def draw_cover(canvas, doc) -> None:
    ...
    canvas.drawRightString(PAGE_W - 16 * mm, 12 * mm, "HEAD: 897ede8")
    canvas.restoreState()


def flow_diagram(steps: list[str], title: str | None = None) -> KeepTogether:
    """Simple horizontal (wrapping) pipeline boxes."""
    ...
    while i < n:
        ...
        widths = []
        usable = CONTENT_W
        box_w = (usable - 12 * (len(chunk) - 1)) / len(chunk) if chunk else usable
        for j, step in enumerate(chunk):
            cells.append(Paragraph(step, box_style))
            widths.append(box_w)
            if j < len(chunk) - 1:
        ...
        t.setStyle(
            TableStyle(
                [
                    ("BACKGROUND", (0, 0), (0, 0), PISTA_SOFT),
                    ("BOX", (0, 0), (0, 0), 0.5, PISTA_ACCENT),
                    ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ...
        # paint every even column as a box
        cmds = [
            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
            ("ALIGN", (0, 0), (-1, -1), "CENTER"),
        ...
        for col in range(0, len(cells), 2):
            cmds.append(("BACKGROUND", (col, 0), (col, 0), PISTA_SOFT))
            cmds.append(("BOX", (col, 0), (col, 0), 0.55, PISTA_ACCENT))
        t.setStyle(TableStyle(cmds))
        rows.append(t)
        i += 4
    items = []
